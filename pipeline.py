...
print(f"iceberg_spark = {iceberg_spark}")
...
df.write \
    .format("iceberg") \
    .mode("append") \
    .saveAsTable("iceberg_test")  # Name of the Iceberg table

# Read data from the Iceberg table
iceberg_df = iceberg_spark.read.format("iceberg").load(f"{iceberg_table_location}/iceberg_test")
